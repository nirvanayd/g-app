...
class CASTELBAJAC {
    ...
    async getName(page) {
        ...
        try {
            return await page.evaluate(() => {
                return document.querySelector('.detail_info_top h3').innerText;
            })
        } catch ( Error ) {
    ...
    async getPrice(page) {
        ...
        try {
            const price = await page.evaluate(() => {
                return document.querySelector('.detail_info_top .price-num').innerText.replace(/\D/g, "");
            })
    ...
    async getImageList(page) {
        ...
        try {
            return await page.evaluate(() => {
                return Array.from(
                    document.querySelectorAll('.slick-track li img')
                ).map((image) => image.src);
            })
        } catch ( Error ) {
